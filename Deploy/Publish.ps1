...
if ($OutputDirectory) {
    Import-Module "$PSScriptRoot/SteamPS"
...
$HelpFile = Get-ChildItem -Path "$PSScriptRoot/SteamPS" -File -Recurse -Filter '*-help.xml'
...
$Nupkg = Get-ChildItem -Path $DeploymentParams['Path'] -Filter 'SteamPS*.nupkg' | ForEach-Object FullName
